...
## Showdown servers
A Showdown server is an HTTP server, that allows communication with the 
Showdown application instance.

### Worker
The only purpose a `worker` instance has is to connect to the message queue
...
It is responsible for authorization and verifying the
correctness & format of the requests, after which a unique PID is assigned to the message
which is immediately returned in the response to the client and the message is
finally queued.

The manager itself does not do any processing of the requests, the message queue is consumed by
